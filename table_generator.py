...
for file in files_to_read:
	...
	name = file[:-4].split("/")[-1]
        ...
        scaled_events, scaled_uncertainties, sel_effs = broc_reader.readfile()
        scaled_datasets.append(scaled_events)
        scaled_datasets_unc.append(scaled_uncertainties)
        for key,value in sel_effs.items():
                sel_effs_dict[key] = value
...
table_creator.writetable(scaled_datasets, scaled_datasets_unc, sel_effs_dict)
